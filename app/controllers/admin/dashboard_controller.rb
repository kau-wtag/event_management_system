class Admin::DashboardController < ApplicationController
  ...
  def index
      ...
      @inactive_users = User.left_joins(:registrations).where(registrations: { id: nil }).or(User.left_joins(:registrations).where('registrations.created_at < ?', 6.months.ago)).distinct || []
      @top_participants = User.joins(:registrations).group('users.id').order('COUNT(registrations.id) DESC').limit(5) || []
      # @users_without_avatars = User.where(avatar: nil) || []
    end
end
